...
//for printing predictor characteristics
	...
	fprintf (stderr, "NHIST= %d; MINHIST= %d; MAXHIST= %d; STORAGESIZE= %d; NBENTRY= %d\n", NHIST, MINHIST, MAXHIST, STORAGESIZE, NBENTRY);
#endif
}
// END PREDICTOR INITIALIZATION
...
int gindex (address_t pc, int bank)
{
	...
	index = pc ^ (pc >> (abs (logg[bank] - bank) + 1)) ^ ch_i[bank].comp ^ F (phist, M, bank);
...
int gindexos (address_t pc, int bank)
{
	int index;
	int M = (m[bank] > 16) ? 16 : m[bank];
	index = pc ^ (pc >> (abs (logg[bank] - bank) + 1)) ^ ch_ios[bank].comp ^ F (phistos, M, bank);
	return (index & ((1 << (logg[bank])) - 1));
}
...
// update bimodal predictor: a hysteresis bit is shared among 4 prediction bits
...
void loopupdate (address_t pc, bool Taken)
{
	if (LHIT >= 0)
	{
		...
		// Loops with more than 2**14 iterations are not treated correctly due to the 14-bit width of the NbIter field.
		// This limitation exists to conserve memory, as larger iteration counts are rare in practical scenarios.
		...
		if (Taken != true)
		{
			if (ltable[LI + LHIT].CurrentIter == ltable[LI + LHIT].NbIter)
			{
				...
				// do not predict when the loop count is 1 or 2     
				{
					// free the entry
					ltable[LI + LHIT].NbIter = 0;
					ltable[LI + LHIT].age = 0;
...
// simple pseudo random number generator: a 2-bit counter, used to avoid ping-pong phenomenom on tagged entry allocations
int MYRANDOM ()
{
	Seed++;
	return (Seed & 3);
...
branch_update *predict (branch_info & b)
{
	...
	if (b.br_flags & BR_CONDITIONAL)
	{
		...
		// look for alternate bank (second longest matching history)
		for (int i = HitBank - 1; i > 0; i--)
		{
			if ((gtable[i][GI[i]].tag == GTAG[i]) && ((USE_ALT_ON_NA < 0) || (abs (2 * gtable[i][GI[i]].ctr + 1) > 1)))
			{
				AltBank = i;
				break;
			}
...
void update (branch_update * u, bool taken, unsigned int target)
{
	...
	if (bi.br_flags & BR_CONDITIONAL)
	{
		...
		if (HitBank > 0)
		{
			// Manage the selection between longest matching and alternate matching for "pseudo"-newly allocated longest matching entry
			...
			if (PseudoNewAlloc)
			{
				if (LongestMatchPred == taken)
					ALLOC = false;
				// if it was delivering the correct prediction, no need to allocate a new entry even if the overall prediction was false
				if (LongestMatchPred != alttaken)
				{
					if ((alttaken == taken) && (USE_ALT_ON_NA < 7))
						USE_ALT_ON_NA++;
		...
		if (ALLOC)
		{
			// if there is some "unuseful" entry to allocate, we allocate an entry with a longer history
			int8_t min = 1;
		...
		if (tage_pred != alttaken)
		{
			if ((tage_pred == taken) && (gtable[HitBank][GI[HitBank]].u < 3))
